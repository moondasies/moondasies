<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:FF006E,25:FF6B9D,50:C471ED,75:8B5CF6,100:06B6D4&height=250&section=header&text=Srijita%20Chatterjee&fontSize=58&fontColor=ffffff&animation=fadeIn&fontAlignY=40&desc=AI%20Product%20Manager%20%7C%20Researcher%20%7C%20Builder&descAlignY=62&descSize=22&descColor=ffffff&stroke=ffffff&strokeWidth=2"/>

</div>

<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=22&duration=2500&pause=800&color=FF6B9D&center=true&vCenter=true&multiline=false&width=750&height=55&lines=AI+Product+Manager+%7C+3+Internships+%F0%9F%9A%80;Building+Sonar+AI+%7C+Voice+Agent+from+Scratch+%F0%9F%8E%99%EF%B8%8F;Research+Published+%40+ICDAM+2025+%F0%9F%8E%93;LLMs+%7C+RAG+%7C+Voice+AI+%7C+Prompt+Engineering+%F0%9F%A7%A0;37K%2B+Followers+%40+SriSphere+%F0%9F%93%B1)](https://git.io/typing-svg)

<br/>

<a href="https://linkedin.com/in/srijita7"><img src="https://img.shields.io/badge/LinkedIn-%230077B5.svg?style=for-the-badge&logo=linkedin&logoColor=white"/></a>&nbsp;
<a href="mailto:srijitachatterjee777@gmail.com"><img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/></a>&nbsp;
<a href="https://instagram.com/sri.sphere"><img src="https://img.shields.io/badge/Instagram-%23E4405F.svg?style=for-the-badge&logo=Instagram&logoColor=white"/></a>&nbsp;
<a href="https://moondasies.github.io"><img src="https://img.shields.io/badge/Portfolio-FF6B9D?style=for-the-badge&logo=firefox&logoColor=white"/></a>

<br/><br/>

![Profile Views](https://komarev.com/ghpvc/?username=moondasies&color=FF006E&style=for-the-badge&label=Profile+Views)
![Followers](https://img.shields.io/github/followers/moondasies?style=for-the-badge&color=8B5CF6&logo=github&logoColor=white)
[![GitHub Pro](https://img.shields.io/badge/GitHub-PRO-gold?style=for-the-badge&logo=github&logoColor=white)](#)

</div>

---

<img align="right" width="260" src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif"/>

## 👩‍💻 About Me

**Srijita Chatterjee** — AI Product Manager 

I sit at the rare intersection of **product strategy** and **AI engineering**. Most PMs understand products. Most engineers understand AI. I understand both — deeply.

With **3 internships** in AI product roles, a **published research paper**, and hands-on experience building real AI systems from scratch, I bring a unique blend of technical depth and product thinking to every team I join.

**What defines my approach:**
- 🎙️ I build the products I manage — not just spec them
- 📄 PRDs that engineers love and stakeholders understand
- 🧠 Deep knowledge of LLMs, RAG, Voice AI & Prompt Engineering
- 📊 Every decision is backed by data, not intuition alone
- 🎓 Published researcher — TheraBot presented at ICDAM 2025
- 📱 Content creator with 37K+ followers on AI & tech

**Currently building:** Sonour AI — a multilingual AI voice agent for sales automation, built entirely from scratch using Claude API + ElevenLabs + Twilio.

---

## 💼 Professional Experience

### 🏢 AI Product Manager Intern — TradeVed
**Feb 2026 – Present | Remote**

> At TradeVed, I lead the development of AI-driven product workflows designed to automate operational processes across internal business systems. My work spans the full product lifecycle — from identifying automation opportunities and defining requirements, to collaborating with engineering teams to ship production-ready AI capabilities.

**Key Contributions:**
- 🔧 Leading AI-driven product workflow development for internal automation
- 📋 Defining product requirements, user flows & feature specifications
- 🤝 Cross-functional collaboration with engineering to implement AI capabilities
- 🎯 Identifying & prioritizing AI integration opportunities across core workflows
- 📈 Enabling scalable decision-making through AI-powered product operations

---

### 🏢 AI Product Manager Intern — Jinn Live
**Nov 2025 – Jan 2026 | Remote**

> At Jinn Live, I took ownership of the AI product experience end-to-end — redesigning prompt structures, building AI pipelines, writing detailed PRDs, and managing enterprise client relationships. My biggest win was moving customer success from 20% to 35% through intelligent prompt workflow redesign.

**Key Contributions:**
- 📈 **Improved customer success rate 20% → 35%** through LLM prompt restructuring
- 🔄 Designed end-to-end AI pipelines: LLM → Presentation → TTS → Audio delivery
- 📝 Authored PRDs, feature specs & user flows reducing engineering ambiguity
- 🏢 Managed enterprise client relationships — feedback collection & issue resolution
- 🚀 Improved product adoption and user satisfaction across client base

---

### 📊 Product Analyst — Pacific Business Solutions
**Aug 2025 – Nov 2025 | On-site**

> In my first product role, I drove measurable business outcomes through customer research, data analysis, and product positioning. I launched the company's first website and introduced AI-driven workflows that improved operational efficiency across teams.

**Key Contributions:**
- 💰 **Increased company sales by 30%** through customer research & product positioning
- 📦 **30% increase in order volume** through market-driven product expansion
- 🌐 Led cross-functional team to launch company's first website end-to-end
- 🤖 Introduced AI-driven workflows improving operational efficiency
- 🔍 Conducted market demand analysis leading to expanded product offerings

---

## 🚀 Projects

### 🏥 Sonour AI — AI Medical Appointment Assistant

[![Status](https://img.shields.io/badge/Status-🚧_Building-FF6B9D?style=for-the-badge)](#)
[![Stack](https://img.shields.io/badge/Stack-FastAPI_+_React_+_MongoDB-8B5CF6?style=for-the-badge)](#)

**The Vision:**
An AI-powered hospital assistant that simulates real patient conversations, performs intelligent symptom triage, routes users to the correct department, and books appointments automatically.

---

## 🧠 What It Does

Sonour AI acts like a **virtual receptionist for hospitals**, capable of:

* Understanding patient symptoms through natural conversation
* Asking follow-up questions (triage logic)
* Determining the correct medical department
* Suggesting doctors based on availability
* Booking appointments in real-time
* Storing full conversation logs for admin review

---

## ⚙️ System Architecture

```
💬 Chat Interface   →  React + TypeScript (Call Simulator UI)
🧠 AI Engine        →  Groq / OpenAI (LLM-based triage + response)
⚡ Backend          →  Python + FastAPI (API + logic layer)
🗄️ Database        →  MongoDB (appointments, conversations, doctors)
📚 Knowledge Base   →  Custom hospital data injected into AI context
```

---

## 🔄 AI Flow

```
1. Patient describes symptoms
2. AI extracts intent + symptoms
3. If unclear → asks follow-up questions
4. Maps symptoms → department
5. Suggests doctor + available slots
6. Books appointment
7. Stores conversation + booking data
```

---

## 🧩 Core Features

* 🧠 AI Triage System — multi-turn symptom analysis
* 🏥 Smart Department Routing — safe fallback to General Physician
* 📅 Appointment Booking Engine — slot-based scheduling
* 👨‍⚕️ Doctor Management — admin-controlled schedules
* 📚 Knowledge Base — dynamic hospital information
* 💬 Call Simulator — test AI without real telephony
* 📊 Dashboard — monitor interactions and bookings

---

## 🛠️ Tech Stack

* **Frontend:** React + TypeScript + Tailwind
* **Backend:** FastAPI (Python)
* **Database:** MongoDB
* **AI:** Groq API (LLaMA models) / OpenAI

---

## ⚠️ Current Status

* ✅ Core backend + frontend structure complete
* ✅ Chat-based AI simulation working
* 🚧 Improving triage accuracy and booking flow
* 🚧 Voice calling integration (future scope)

---

## 🚀 Future Scope

* 📞 Real-time voice integration (Twilio)
* 🌐 Multi-language support (Hindi + English)
* 📈 Advanced analytics dashboard
* 🏥 Multi-hospital SaaS deployment

---

[![View Project](https://img.shields.io/badge/GitHub-View_Project-FF6B9D?style=for-the-badge\&logo=github\&logoColor=white)](https://github.com/moondasies/sonourAI)

---

### 📊 Banking Transaction Analysis
[![Status](https://img.shields.io/badge/Status-✅_Complete-4CAF50?style=for-the-badge)](#)
[![Stack](https://img.shields.io/badge/Stack-Python_+_Power_BI_+_Tableau-F7931E?style=for-the-badge)](#)

**The Vision:** Deep analysis of banking transaction datasets to surface spending patterns, detect anomalies, and enable fraud detection.

```
🐍 Analysis      →  Python, Pandas, NumPy
📊 Visualization →  Matplotlib, Seaborn, Plotly
📈 BI Dashboards →  Power BI, Tableau, Excel
🔍 Output        →  Fraud detection + customer segmentation models
```

**Deliverables:**
- 📊 Interactive dashboards for financial insight visualization
- 🚨 Anomaly detection models for fraud identification
- 👥 Customer segmentation analysis
- 💡 Behavioral spending pattern insights

---
## 🛠️ Complete Tech Stack

---

### 🤖 AI & Machine Learning
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white)
![OpenAI](https://img.shields.io/badge/OpenAI_API-412991?style=for-the-badge&logo=openai&logoColor=white)
![Groq](https://img.shields.io/badge/Groq_LLM-FF4F00?style=for-the-badge)
![Claude](https://img.shields.io/badge/Claude_API-FF006E?style=for-the-badge)
![LangChain](https://img.shields.io/badge/LangChain-0FA958?style=for-the-badge)
![LlamaIndex](https://img.shields.io/badge/LlamaIndex-8B5CF6?style=for-the-badge)
![RAG](https://img.shields.io/badge/RAG_Pipelines-111827?style=for-the-badge)
![Whisper](https://img.shields.io/badge/Whisper_STT-412991?style=for-the-badge&logo=openai&logoColor=white)
![ElevenLabs](https://img.shields.io/badge/ElevenLabs_TTS-000000?style=for-the-badge)
![WebSockets](https://img.shields.io/badge/WebSockets-010101?style=for-the-badge)

---

### ⚡ AI Tools & Builders
![Antigravity](https://img.shields.io/badge/Antigravity_AI-FF6B9D?style=for-the-badge)
![Cursor](https://img.shields.io/badge/Cursor_AI-111827?style=for-the-badge)
![Stitch](https://img.shields.io/badge/Stitch_Google_AI-4285F4?style=for-the-badge)
![Make](https://img.shields.io/badge/Make.com-Automation-5A67D8?style=for-the-badge)
![Zapier](https://img.shields.io/badge/Zapier-Automation-FF4A00?style=for-the-badge)
![n8n](https://img.shields.io/badge/n8n_Workflows-EA4B71?style=for-the-badge)
![Runway](https://img.shields.io/badge/Runway_AI-000000?style=for-the-badge)
![Midjourney](https://img.shields.io/badge/Midjourney-000000?style=for-the-badge)
![DALL·E](https://img.shields.io/badge/DALL·E-412991?style=for-the-badge)

---

### 📊 Data & Analytics
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy)
![Scikit Learn](https://img.shields.io/badge/Scikit_Learn-F7931E?style=for-the-badge)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=for-the-badge)
![Plotly](https://img.shields.io/badge/Plotly-3F4F75?style=for-the-badge)
![Power BI](https://img.shields.io/badge/Power_BI-F2C811?style=for-the-badge)
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=for-the-badge)

---

### 🧠 Product & Analytical Skills
![A/B Testing](https://img.shields.io/badge/A%2FB_Testing-10B981?style=for-the-badge)
![User Research](https://img.shields.io/badge/User_Research-6366F1?style=for-the-badge)
![Product Thinking](https://img.shields.io/badge/Product_Thinking-EC4899?style=for-the-badge)
![Data Analysis](https://img.shields.io/badge/Data_Analysis-0EA5E9?style=for-the-badge)
![Problem Solving](https://img.shields.io/badge/Problem_Solving-22C55E?style=for-the-badge)
![Experimentation](https://img.shields.io/badge/Experimentation-F59E0B?style=for-the-badge)
![Hypothesis Testing](https://img.shields.io/badge/Hypothesis_Testing-8B5CF6?style=for-the-badge)
![Metrics Design](https://img.shields.io/badge/Product_Metrics-14B8A6?style=for-the-badge)

---

### 💼 Product Management Tools
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=for-the-badge&logo=figma)
![Notion](https://img.shields.io/badge/Notion-000000?style=for-the-badge&logo=notion)
![Jira](https://img.shields.io/badge/Jira-0052CC?style=for-the-badge&logo=jira)
![Asana](https://img.shields.io/badge/Asana-273347?style=for-the-badge&logo=asana)
![Canva](https://img.shields.io/badge/Canva-00C4CC?style=for-the-badge&logo=canva)
![Miro](https://img.shields.io/badge/Miro-FFD02F?style=for-the-badge)

---

### 🔧 Development & DevOps
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb)
![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge)
![SQLite](https://img.shields.io/badge/SQLite-003B57?style=for-the-badge)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker)
![Vercel](https://img.shields.io/badge/Vercel-000000?style=for-the-badge)
![Render](https://img.shields.io/badge/Render-46E3B7?style=for-the-badge)

---

## 🧠 AI PM Expertise Depth

```
🔵 Large Language Models (LLMs)
   → How they work, when to use, cost-performance tradeoffs
   → Production evaluation: accuracy, latency, hallucination rate
   → Model selection for different product use cases

🟣 Retrieval Augmented Generation (RAG)
   → Architecture design, embedding strategies, vector DBs
   → Chunking strategies, retrieval optimization
   → RAG vs fine-tuning decision frameworks

🩷 Prompt Engineering
   → Production-grade prompts for real-world AI products
   → Chain-of-thought, few-shot, system prompt design
   → Iterative prompt testing and optimization

🎙️ Voice AI Products
   → End-to-end STT → LLM → TTS pipeline design
   → Real-time audio streaming with WebSockets
   → Latency optimization for conversational voice AI

📋 AI Product Management
   → PRD writing for AI features
   → AI roadmapping and prioritization
   → Stakeholder communication for AI products
   → AI ethics, bias, responsible AI in product decisions

📊 Product Analytics
   → A/B testing design and analysis
   → User research and persona development
   → Data-driven feature prioritization (RICE, MoSCoW)
   → OKRs and metric frameworks for AI products
```

---

## 🏆 Certifications & Achievements

<div align="center">

| 🏅 | Achievement | Issuer | Year |
|---|---|---|---|
| 🎓 | **Microsoft AI Product Management Certification** | Microsoft | 2025 |
| 🐍 | **Python Data Science & Generative AI** | Certification Body | 2025 |
| 📄 | **Research Paper — TheraBot AI** | ICDAM 2025 Conference | 2025 |
| ⭐ | **GitHub Pro** | GitHub | 2026 |
| 📱 | **37K+ Followers — SriSphere** | Instagram | Ongoing |

</div>

---

## 📱 SriSphere — Content Creator

<div align="center">

Beyond product and engineering, I run **[@sri.sphere](https://instagram.com/sri.sphere)** — an AI & tech content channel with **37,000+ followers** 🎉

</div>

**Content I create:**
- 🤖 AI tools — practical guides and honest reviews
- 🏗️ Building AI products in public — real behind-the-scenes
- 💼 AI PM career path — what it actually takes
- 📚 Making complex tech accessible for everyone
- 🚀 Startup building and learning journey

[![SriSphere](https://img.shields.io/badge/@sri.sphere-37K+_Followers-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://instagram.com/sri.sphere)

---

## 📊 GitHub Statistics

<div align="center">

<img width="49%" src="https://github-readme-stats.vercel.app/api?username=moondasies&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true&hide_border=true&bg_color=0D1117&title_color=FF6B9D&icon_color=8B5CF6&text_color=ffffff&border_radius=10"/>

<img width="49%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=moondasies&layout=compact&langs_count=8&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=FF6B9D&text_color=ffffff&border_radius=10"/>

</div>

<div align="center">

[![GitHub Streak](https://streak-stats.demolab.com/?user=moondasies&theme=tokyonight&hide_border=true&background=0D1117&ring=FF006E&fire=C471ED&currStreakLabel=FF6B9D&border_radius=10)](https://git.io/streak-stats)

</div>

---

## 🎯 Current Status

```python
class SrijitaChatterjee:
    name       = "Srijita Chatterjee"
    role       = "AI Product Manager"
    experience = "3 AI/Product Internships"
    research   = "Published @ ICDAM 2025"
    
    currently_building = [
        "Sonar AI — Multilingual Voice Agent 🎙️",
        "Portfolio of AI PM case studies 📋"
    ]
    
    learning = [
        "FastAPI & AI system architecture",
        "Advanced RAG pipelines",
        "AI product metrics & evaluation"
    ]
    
    open_to = [
        "AI Product Manager",
        "Associate Product Manager (APM)",
        "Product Manager",
        "Technical Product Manager"
    ]
    
    location   = "India 🇮🇳 — Remote / Hybrid / Onsite"
    contact    = "srijitachatterjee777@gmail.com"
    content    = "@sri.sphere — 37K+ followers"
    mission    = "Build AI products that actually matter 🚀"
```

---

## 🌸 Let's Connect

<div align="center">

### Open to AI PM, APM & Product Manager Roles Across India 🇮🇳

*If you're building something in AI, looking for a PM who can also build, or just want to talk LLMs and voice AI — let's connect!*

<br/>

[![LinkedIn](https://img.shields.io/badge/Connect_on_LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/srijita7)
[![Email](https://img.shields.io/badge/Email_Me-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:srijitachatterjee777@gmail.com)
[![Portfolio](https://img.shields.io/badge/View_Portfolio-FF6B9D?style=for-the-badge&logo=firefox&logoColor=white)](https://moondasies.github.io)
[![Instagram](https://img.shields.io/badge/Follow_SriSphere-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://instagram.com/sri.sphere)

<br/>

**⭐ If you find my work interesting, please star my repositories! ⭐**

</div>

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:06B6D4,25:8B5CF6,50:C471ED,75:FF6B9D,100:FF006E&height=140&section=footer&text=Building+AI+Products+That+Matter+%F0%9F%9A%80&fontSize=22&fontColor=ffffff&animation=twinkling&fontAlignY=65"/>

</div>

<!--
**moondasies/moondasies** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
